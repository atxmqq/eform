<x-layout title="{{ __('ui.auth.system_name') }}">
    <div class="min-h-[70vh] flex items-center justify-center">
        <div class="bg-white rounded-2xl shadow-lg border border-gray-100 p-10 w-full max-w-md">
            <div class="text-center mb-8">
                <div class="text-5xl mb-3">📋</div>
                <h1 class="text-2xl font-bold text-gray-800">{{ __('ui.auth.system_name') }}</h1>
                <p class="text-gray-500 text-sm mt-1">{{ __('ui.auth.signin_institution') }}</p>
            </div>
            ...
            <div class="space-y-3">
                ...
                <a href="{{ route('auth.social.redirect', 'microsoft') }}"
                   class="flex items-center justify-center gap-3 w-full px-4 py-3 bg-white border-2 border-gray-200 rounded-xl text-gray-700 font-medium hover:border-blue-400 hover:bg-blue-50 transition">
                    ...
                    {{ __('ui.auth.signin_microsoft') }}
                </a>
            </div>

            <p class="text-center text-xs text-gray-400 mt-6">
                {{ __('ui.auth.institution_only') }}
            </p>
            ...
            <div class="mt-6 pt-5 border-t border-dashed border-gray-200">
                ...
                <form method="POST" action="{{ route('login.dev') }}" class="space-y-3">
                    @csrf
                    <input type="email" name="email" placeholder="Email" value="{{ old('email') }}" required
                           class="block w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:border-blue-500 focus:ring-2 focus:ring-blue-500 @error('email') border-red-400 @enderror">
                    @error('email')
